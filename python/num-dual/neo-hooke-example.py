...
import numpy as np
from interface import d2f_dT2_num_dual, df_dT_num_dual
...
if __name__ == "__main__":
    ...
    # dual numbers computations
    tic = time.time()
    dPsi_dC_num_dual = df_dT_num_dual(lambda C: psi(C, K, G), C)
    d2Psi_dC_dC_num_dual = d2f_dT2_num_dual(lambda C: psi(C, K, G), C)
    toc = time.time()
    ...
    print(f"Time taken for automatic differentiation with dual numbers: {time_num_dual:.4e} seconds")
